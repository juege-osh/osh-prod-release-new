...
- 左侧有 `操作手册`。
- Change 详情顶部有 `上线总览`，能看到整体进度、总节点、失败、回滚、报告、双评审。
- `上线后结果` 能看到是否已切绿、是否人工验证、是否同步蓝、是否回蓝/回滚。
- Change 详情里有 `上线项`、`新增动作`、`审批测试`、`报告闸门`、`操作链`。
- 每个节点卡片有 `分析`、`dry-run`、`执行`、`验证`、`回滚`、`编辑`。

## 1.1 看懂上线总览

打开任意 Change，先看详情顶部的 `上线总览`。

重点看这些：

- `整体进度`：按单项操作、报告、双评审、切绿、人工验证和同步蓝计算。
- `总节点`：这次上线被拆成了多少个节点。
- `失败`：只要不是 0，先别继续上线。
- `回滚`：出现数字说明已有回蓝或回滚动作，要先复查操作链。
- `报告`：必须是 `5/5`，也就是规范、环境差异、announce、功能、数据都通过。
- `双评审`：必须覆盖每个上线项。
- `单项操作覆盖`：分析、dry-run、执行、验证都要等于上线项总数。
- `重点保护`：课程模块和用户模块默认应该显示 `未发现变化`。

如果这里显示缺口，先点对应 tab 补齐，不要直接切绿。
